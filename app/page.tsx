...
type TriageResult = { level: string; tone: 'urgent' | 'soon' | 'routine'; summary: string; specialty: string; concerns: string[] }
...
export default function Page() {
  ...
  const analyzeSymptoms = () => {
    setAnalyzing(true)
    window.setTimeout(() => {
      const note = transcript.toLowerCase()
      const urgent = ['chest pain', 'difficulty breathing', 'shortness of breath', 'unconscious', 'stroke', 'severe bleeding', 'one side weakness'].some((term) => note.includes(term))
      const soon = ['high fever', 'persistent vomiting', 'severe pain', 'blood', 'dizziness', 'dehydration'].some((term) => note.includes(term))
      setTriage({ level: urgent ? 'Urgent' : soon ? 'Needs attention soon' : 'Routine', tone: urgent ? 'urgent' : soon ? 'soon' : 'routine', summary: urgent ? 'Some symptoms may need immediate medical attention.' : soon ? 'A clinician should review these symptoms soon.' : 'These symptoms can usually start with a primary-care consultation.', specialty: urgent ? 'Emergency care' : note.includes('cough') || note.includes('breath') ? 'General Physician' : 'General Physician', concerns: urgent ? ['Possible red-flag symptom detected', 'Do not wait for an online consultation'] : soon ? ['Symptoms may need an in-person assessment', 'Keep track of changes and temperature'] : ['Share duration, severity, and medicines taken', 'A doctor can guide the next step'] })
      setAnalyzing(false)
    }, 700)
  }
...
function SymptomTriage({ transcript, setTranscript, listening, setListening, voiceError, triage, analyzing, analyzeSymptoms, setView, setBookedDoctor, bookedDoctor }: { transcript: string; setTranscript: (v: string) => void; listening: boolean; setListening: () => void; voiceError: string; triage: TriageResult | null; analyzing: boolean; analyzeSymptoms: () => void; setView: (v: View) => void; setBookedDoctor: (v: string | null) => void; bookedDoctor: string | null }) {
  const urgent = triage?.tone === 'urgent'
  return <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_380px]"><div className="rounded-3xl border border-border bg-card p-6 sm:p-10"><div className="mx-auto max-w-md text-center"><button onClick={setListening} className={`mx-auto grid size-32 place-items-center rounded-full text-primary-foreground shadow-xl transition ${listening ? 'bg-accent mic-ring' : 'bg-primary hover:scale-105'}`} aria-label={listening ? 'Stop listening' : 'Start listening'}><Mic className="size-11" /></button><h2 className="mt-7 font-display text-xl font-bold">{listening ? 'Listening to you…' : transcript ? 'Your symptom note is ready' : 'Tell us what you feel'}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Speak naturally, then review your note before MedThread suggests the right next step.</p><textarea value={transcript} onChange={(e) => setTranscript(e.target.value)} rows={4} placeholder="Example: I have had a fever and cough since yesterday…" className="mt-6 w-full resize-none rounded-2xl border border-border bg-background p-4 text-left text-sm leading-6 outline-none focus:ring-2 focus:ring-primary/30" aria-label="Describe your symptoms" />{voiceError && <p role="alert" className="mt-3 rounded-xl bg-accent/20 px-3 py-2 text-left text-xs font-semibold text-accent-foreground">{voiceError}</p>}<button disabled={!transcript.trim() || analyzing} onClick={analyzeSymptoms} className="mt-4 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">{analyzing ? 'Reviewing your note…' : 'Analyze symptoms'}<Activity className="size-4" /></button><p className="mt-4 text-xs text-muted-foreground">This is guidance, not a diagnosis. For emergencies, call local emergency services.</p></div></div><div className="rounded-3xl bg-primary p-6 text-primary-foreground"><ShieldCheck className="size-7 text-accent" /><h2 className="mt-6 font-display text-xl font-bold">A safer next step</h2>{!triage ? <p className="mt-3 text-sm leading-6 text-primary-foreground/75">We&apos;ll look for urgency signals, suggest a care type, and help you connect with a suitable nearby doctor.</p> : <div className="mt-5"><div className={`rounded-2xl p-4 ${urgent ? 'bg-destructive/20' : 'bg-primary-foreground/10'}`}><div className="flex items-center gap-2 font-bold"><span className={`size-2.5 rounded-full ${urgent ? 'bg-red-400 animate-pulse' : 'bg-accent'}`} />{triage.level}</div><p className="mt-2 text-sm leading-6 text-primary-foreground/80">{triage.summary}</p></div><div className="mt-5 flex flex-col gap-3">{triage.concerns.map((concern) => <div key={concern} className="flex gap-2 text-sm text-primary-foreground/80"><CircleCheck className="mt-0.5 size-4 shrink-0 text-accent" />{concern}</div>)}</div>{urgent ? <p className="mt-6 text-sm font-bold text-accent">Please seek emergency care now rather than waiting for an online booking.</p> : <><p className="mt-6 text-sm text-primary-foreground/70">Suggested care: <strong className="text-primary-foreground">{triage.specialty}</strong></p><button onClick={() => { setView('doctors'); setBookedDoctor(null) }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-accent py-3 font-bold text-accent-foreground"><Stethoscope className="size-4" />Find a suitable doctor</button></>}</div>}{bookedDoctor && <div className="mt-4 rounded-xl bg-accent/20 p-3 text-sm font-bold text-accent">Consultation request started with {bookedDoctor}.</div>}</div></div>
}

function AppView({ view, query, setQuery, located, getLocation, matched, findDoctor, listening, setListening, transcript, setTranscript, voiceError, triage, analyzing, analyzeSymptoms, setView, setBookedDoctor, bookedDoctor }: { view: View; query: string; setQuery: (v: string) => void; located: boolean; getLocation: () => void; matched: boolean; findDoctor: () => void; listening: boolean; setListening: () => void; transcript: string; setTranscript: (v: string) => void; voiceError: string; triage: TriageResult | null; analyzing: boolean; analyzeSymptoms: () => void; setView: (v: View) => void; setBookedDoctor: (v: string | null) => void; bookedDoctor: string | null }) {
