...
type View = 'home' | 'doctors' | 'symptoms' | 'tests' | 'medicines'
  ...
  return (
    <div className={dark ? 'dark min-h-screen' : 'min-h-screen'}>
      <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
        <header className="sticky top-0 z-50 border-b border-border/70 bg-background/90 backdrop-blur-xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="hidden items-center gap-2 rounded-full bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/15 sm:flex"><Bell className="size-4" />My care</button>
        ...
        {view === 'home' ? <Home setView={setView} listening={listening} setListening={setListening} /> : <AppView view={view} query={query} setQuery={setQuery} located={located} getLocation={getLocation} matched={matched} findDoctor={findDoctor} listening={listening} setListening={setListening} />}
...
function AppView({ view, query, setQuery, located, getLocation, matched, findDoctor, listening, setListening }: { view: View; query: string; setQuery: (v: string) => void; located: boolean; getLocation: () => void; matched: boolean; findDoctor: () => void; listening: boolean; setListening: (v: boolean) => void }) {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-16"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><button className="mb-5 text-sm font-semibold text-primary" onClick={() => window.history.back()}>← Back to overview</button><div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-2xl bg-secondary text-primary"><Icon className="size-6" /></span><div><p className="eyebrow">MedThread care hub</p><h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">{config[0]}</h1></div></div><p className="mt-4 max-w-xl text-lg text-muted-foreground">{config[1]}</p></div>{view === 'doctors' && <button onClick={getLocation} className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2.5 text-sm font-bold text-primary"><LocateFixed className="size-4" />{located ? 'Location found' : 'Use my location'}</button>}</div>
    ...
    {view === 'symptoms' && <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_380px]"><div className="rounded-3xl border border-border bg-card p-6 sm:p-10"><div className="mx-auto max-w-md text-center"><button onClick={() => setListening(!listening)} className={`mx-auto grid size-32 place-items-center rounded-full text-primary-foreground shadow-xl transition ${listening ? 'bg-accent mic-ring' : 'bg-primary hover:scale-105'}`} aria-label="Start listening"><Mic className="size-11" /></button><h2 className="mt-7 font-display text-xl font-bold">{listening ? 'Listening to you…' : 'Start with your voice'}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{listening ? 'Describe your symptoms in Hindi or English. Take your time.' : 'Tap the microphone and describe what you are feeling naturally.'}</p><div className="mt-7 flex flex-wrap justify-center gap-2">{['Headache', 'Fever', 'Cough', 'Stomach pain'].map((x) => <button onClick={() => setListening(true)} key={x} className="rounded-full border border-border px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:border-primary hover:text-primary">{x}</button>)}</div></div></div><div className="rounded-3xl bg-primary p-6 text-primary-foreground"><ShieldCheck className="size-7 text-accent" /><h3 className="mt-5 font-display text-xl font-bold">A safe first step</h3><p className="mt-3 text-sm leading-6 text-primary-foreground/75">Our triage assistant organizes your symptoms for a doctor. It does not diagnose or replace professional medical advice.</p><div className="mt-8 space-y-3 text-sm">{['Speak in your preferred language', 'Get urgency guidance', 'Share a clear summary with a doctor'].map((x) => <p key={x} className="flex items-center gap-2"><CircleCheck className="size-4 text-accent" />{x}</p>)}</div></div></div>}
